...
export const config = {
  ...
  projectRootDirectory: {
    ...
    description:
      'Directory to search for the eclipse configuration file (.cproject). Leave empty to use the atom project root directory.',
    type: 'string',
    default: '',
    order: 2
  },
  touchbarButtons: {
    ...
    description: 'Show buttons on the TouchBar to build and clean the project',
    type: 'boolean',
    default: false,
    order: 3
  },
...
const searchProjectFiles = (dir, recursively = false, list = []) => {
    ...
    try {
      if (existsSync(fullName)) {
        if (statSync(fullName).isDirectory()) {
          if (recursively) {
            list = searchProjectFiles(fullName, recursively, list);
          }
...
const searchProjectFiles = (dir, done) => {
  let results = [];
  readdir(dir, (err, list) => {
    ...
    list.forEach(file => {
      const fullName = path.join(dir, file);
      stat(fullName, (statErr, statRes) => {
        if (statRes && statRes.isDirectory()) {
          searchProjectFiles(fullName, (searchErr, res) => {
